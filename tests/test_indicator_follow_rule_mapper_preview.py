...
class IndicatorFollowRuleMapperPreviewTest(unittest.TestCase):
    ...
    def test_buy_macd_position_candidate_uses_existing_condition_shape(self):
        state = deepcopy(self.ui_state)
        state["buy_ui"]["signal_filter"]["buy_ocr_value_line"] = ""
        ...
        candidate = result["preview_rules"]["indicator_follow_rule_preview"]["candidates"]["buy"]
        condition = candidate["add_conditions"][0]
    ...
    def test_buy_macd_position_disabled_does_not_create_buy_candidate(self):
        state = deepcopy(self.ui_state)
        state["buy_ui"]["signal_filter"]["buy_ocr_value_line"] = ""
        state["buy_ui"]["signal_filter"].update({
            "buy_macd_position_enabled": False,
            "buy_macd_position_target": "MACD",
            "buy_macd_position_compare_target": "SIGNAL",
            "buy_macd_position_operator": "<=",
        })
    ...
    def test_buy_macd_position_same_condition_has_no_commit_diff(self):
        state = deepcopy(self.ui_state)
        state["buy_ui"]["signal_filter"]["buy_ocr_value_line"] = ""
        state["buy_ui"]["signal_filter"].update({
            "buy_macd_position_enabled": True,
