...
class ExecutionRuntimeAPIContractTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _api(self) -> ExecutionRuntimeAPI:
        return ExecutionRuntimeAPI(
            ...
            metadata={"api": {"name": "contract"}},
        )

    def _order(self) -> dict:
        return {
            "id": "ORDER_API_CONTRACT_1",
            "status": "REAL_READY",
            "source_signal_id": "SIG_API_CONTRACT_1",
            "code": "003550",
            "side": "BUY",
            "quantity": 10,
            "price": 85000,
    ...
    def test_submit_dry_run_ready_contract(self) -> None:
        api = self._api()
    ...
    def test_submit_dry_run_blocked_contract(self) -> None:
        api = self._api()
    ...
    def test_submit_dry_run_invalid_contract(self) -> None:
        api = self._api()
    ...
    def test_submit_dry_run_uses_only_coordinator_submit(self) -> None:
        api = self._api()
        order = self._order()
        guard = self._guard()
        confirmations = self._confirmations()

        with mock.patch.object(api.coordinator, "submit", wraps=api.coordinator.submit) as coordinator_submit:
            result = api.submit_dry_run(order, guard, confirmations)

        self.assertEqual("READY", result["status"])
        coordinator_submit.assert_called_once_with(order, guard, confirmations)

    def test_get_last_result_returns_deepcopy(self) -> None:
        api = self._api()
        api.submit_dry_run(self._order(), self._guard(), self._confirmations())

        first = api.get_last_result()
        first["status"] = "MUTATED"
        first["coordinator_result"]["status"] = "MUTATED_COORDINATOR"

        second = api.get_last_result()
        self.assertEqual("READY", second["status"])
        self.assertEqual("READY", second["coordinator_result"]["status"])
    ...
    def test_list_sessions_list_events_summary_contract(self) -> None:
        api = self._api()
        api.submit_dry_run(self._order(), self._guard(), self._confirmations())
        session = api.list_sessions()[0]

        sessions = api.list_sessions()
        events = api.list_events(session.session_id)
        summary = api.summary()
        ...
        self.assertFalse(summary["runtime_write"])
        self.assertNotIn("mutated", api.list_sessions()[0].metadata)
        self.assertEqual("CREATED", api.list_events(session.session_id)[0]["payload"]["status"])
    ...
    def test_gui_commit_queue_send_order_connections_absent_contract(self) -> None:
    ...
    def test_runtime_files_and_rules_unchanged_contract(self) -> None:
        ...
        api = self._api()
        api.submit_dry_run(self._order(), self._guard(), self._confirmations())
        api.submit_dry_run(self._order(), self._guard(), confirmations={})
        snapshot = api.snapshot()
