...
class AutoTradeOperationModeE2ETest(unittest.TestCase):
    ...
    def test_multi_selection_keeps_existing_per_stock_result_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            first = self._stock(
                root,
                mode="CONTINUOUS",
                with_ats=True,
                config_values={"end_buy_time": "13:30:00"},
            )
            second = root / "stocks" / "222222_두번째종목"
            second.mkdir(parents=True)
            (second / "config.json").write_text(
        ...
        self.assertEqual("SCHEDULED", first_config["operation_mode"])
        self.assertNotIn("manual_ats_selection", first_state)
        self.assertEqual("CONTINUOUS", second_config["operation_mode"])
        self.assertIn("manual_ats_selection", second_state)
        self.assertIn("실패 1개", window.statusBarMessage.call_args.args[0])
