...
class IndicatorFollowSettingsValidationOnCommitTest(unittest.TestCase):
    ...
    def test_sell_price_box_legacy_sign_and_explicit_round_trip(self) -> None:
        source = json.loads(self.source_rules_path.read_text(encoding="utf-8"))
        ...
        for direction, compare, expected_sign in (
            ...
            with self.subTest(direction=direction, compare=compare):
                ...
                legacy_condition["price_box_direction_combo"] = direction
                legacy_condition["price_box_compare_combo"] = compare
                legacy_condition.pop("price_box_sign_combo", None)
                with tempfile.TemporaryDirectory() as temp_dir:
                    rules_path = Path(temp_dir) / "rules.json"
                    rules_path.write_text(
        ...
        condition_b["price_box_direction_combo"] = "하향"
        condition_b["price_box_sign_combo"] = "-"
        condition_b["price_box_value_line"] = "0.1"
        condition_b["price_box_compare_combo"] = "이상"
        with tempfile.TemporaryDirectory() as temp_dir:
            rules_path = Path(temp_dir) / "rules.json"
            rules_path.write_text(
                json.dumps(source, ensure_ascii=False, indent=2) + "\n",
            ...
            try:
                self.assertEqual(
                    ("하향", "-", "0.1", "이상"),
                    (
                        reloaded.sell_signal_condition_b_price_box_direction_combo.currentText(),
                        reloaded.sell_signal_condition_b_price_box_sign_combo.currentText(),
                        reloaded.sell_signal_condition_b_price_box_value_line.text(),
                        reloaded.sell_signal_condition_b_price_box_compare_combo.currentText(),
                    ),
                )
            finally:
                reloaded.close()
