...
class _AcceptedAtsDialog:
    requested_sell_method = ""
...
class AutoTradeMultiTargetContextOpsTest(unittest.TestCase):
    ...
    def test_ats_dialog_keeps_initial_target_snapshot(self) -> None:
        initial_targets = [
            (Path("C:/stocks/111111"), "111111", "첫번째"),
            (Path("C:/stocks/222222"), "222222", "두번째"),
        ]
        changed_selection = [(Path("C:/stocks/333333"), "333333", "세번째")]
        window = MagicMock()
        window.selected_stock_infos.side_effect = [initial_targets, changed_selection]
        ...
        configs = {
            str(initial_targets[0][0] / "config.json"): {
                "operation_mode": "CONTINUOUS"
            },
            str(initial_targets[1][0] / "config.json"): {
                "operation_mode": "CONTINUOUS"
            },
        }
        ...
        self.assertEqual(1, window.selected_stock_infos.call_count)

    def test_ats_dialog_with_no_manual_targets_exits_silently(self) -> None:
        targets = [
            (Path("C:/stocks/111111"), "111111", "시간1"),
            (Path("C:/stocks/222222"), "222222", "시간2"),
        ]
        window = MagicMock()
        window.selected_stock_infos.return_value = targets

        with (
            patch.object(
                ats_ops,
                "read_json_dict",
                return_value={"operation_mode": "SCHEDULED"},
            ),
            patch.object(ats_ops, "ManualAtsSettingsDialog") as dialog,
        ):
            ats_ops.auto_trade_open_selected_manual_ats_settings_dialog(window)
    ...
    def test_real_auto_trade_window_rejects_mixed_selection_without_toast_or_dialog(
        self,
    ) -> None:
        ...
        with (
            patch.object(window, "selected_stock_infos", return_value=targets),
            patch.object(window, "selected_manual_ats_state", return_value={}),
            patch.object(
                ats_ops,
                "read_json_dict",
                side_effect=lambda path: configs.get(str(path), {}),
            ...
            patch.object(window, "save_selected_manual_ats_state") as save_state,
        ):
            ats_ops.auto_trade_open_selected_manual_ats_settings_dialog(window)

        self.assertIsNone(getattr(window, "_common_toast_message", None))
        dialog.assert_not_called()
        save_state.assert_not_called()
        window.close()

    def test_real_auto_trade_window_silently_rejects_no_manual_targets(self) -> None:
        from gui_auto_trade_setting_window import AutoTradeSettingWindow
        ...
        toast = getattr(window, "_common_toast_message", None)
