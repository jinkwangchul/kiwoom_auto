...
class ExecutionRuntimeFileInitOpenPolicyContractTest(unittest.TestCase):
    ...
    def _orchestrator(self, status_case: str = "READY", *, init_commit_ready: bool | None = None) -> dict:
        if status_case == "READY":
            preview = build_execution_runtime_file_init_preview(
                self.project_order_executions,
                self.project_order_locks,
            ...
            existing = self.temp_root / "order_executions.json"
            existing.write_text("{}", encoding="utf-8")
            preview = build_execution_runtime_file_init_preview(existing, self.temp_root / "order_locks.json")
            approval = approve_execution_runtime_file_init(
                preview,
                manual_runtime_file_init_confirmed=True,
            )
        elif status_case == "INVALID":
            preview = build_execution_runtime_file_init_preview("", self.project_order_locks)
            approval = approve_execution_runtime_file_init(
                preview,
                manual_runtime_file_init_confirmed=True,
            )
        ...
        if init_commit_ready is not None:
            result["init_commit_ready"] = init_commit_ready
        return result
    ...
    def test_file_init_allowed_only_when_ready(self) -> None:
        cases = [
            self._evaluate(),
            self._evaluate(file_init_commit_plan_orchestrator_result=self._orchestrator("BLOCKED")),
            self._evaluate(file_init_commit_plan_orchestrator_result=self._orchestrator("INVALID")),
            self._evaluate(file_init_commit_plan_orchestrator_result=self._orchestrator("SKIPPED")),
            self._evaluate(file_init_commit_plan_orchestrator_result=self._orchestrator(init_commit_ready=False)),
        ]

        self.assertTrue(cases[0]["file_init_allowed"])
        for result in cases[1:]:
            self.assertFalse(result["file_init_allowed"])

    def test_orchestrator_statuses_are_preserved(self) -> None:
        self.assertEqual(
            "BLOCKED",
            self._evaluate(file_init_commit_plan_orchestrator_result=self._orchestrator("BLOCKED"))["status"],
        )
        self.assertEqual(
            "INVALID",
            self._evaluate(file_init_commit_plan_orchestrator_result=self._orchestrator("INVALID"))["status"],
        )
        self.assertEqual(
            "SKIPPED",
            self._evaluate(file_init_commit_plan_orchestrator_result=self._orchestrator("SKIPPED"))["status"],
        )

    def test_init_commit_ready_false_blocks(self) -> None:
        result = self._evaluate(file_init_commit_plan_orchestrator_result=self._orchestrator(init_commit_ready=False))

        self.assertEqual("BLOCKED", result["status"])
        self.assertIn("INIT_COMMIT_READY_IS_NOT_TRUE", result["issues"])

    def test_runtime_confirmation_missing_blocks(self) -> None:
    ...
    def test_project_confirmation_missing_blocks(self) -> None:
    ...
    def test_runtime_file_init_enabled_missing_blocks(self) -> None:
    ...
    def test_project_runtime_file_init_allow_missing_blocks(self) -> None:
    ...
    def test_preview_only_and_runtime_write_contract(self) -> None:
        results = [
            self._evaluate(),
    ...
    def test_no_file_write_mkdir_or_external_calls(self) -> None:
        with (
    ...
    def test_actual_runtime_files_and_rules_unchanged(self) -> None:
        ...
        self._evaluate()
        self._evaluate(confirmations=self._confirmations(runtime_confirmed=False))
        self._evaluate(environment_flags=self._environment(enabled=False))
        self._evaluate(file_init_commit_plan_orchestrator_result=None)
