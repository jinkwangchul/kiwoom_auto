...
class PostExecutionReviewContractTest(unittest.TestCase):
    def _lock_release_record(self, **overrides: object) -> dict[str, object]:
        result: dict[str, object] = {
            ...
            "order_id": "ORDER_POST_REVIEW_CONTRACT_1",
            "request_hash": "HASH_POST_REVIEW_CONTRACT_1",
            "broker_order_no": "BRK_POST_REVIEW_CONTRACT_1",
        }
    ...
    def test_all_valid_completes_execution_contract(self) -> None:
        ...
        self.assertEqual("ORDER_POST_REVIEW_CONTRACT_1", result["lock_release_record"]["order_id"])

    def test_blocked_invalid_and_malformed_inputs_contract(self) -> None:
        blocked = review_post_execution(self._lock_release(status="BLOCKED", issues=["LOCK_BLOCKED"]))
        ...
        self.assertFalse(blocked["execution_completed"])
        self.assertIn("LOCK_BLOCKED", blocked["issues"])
    ...
    def test_missing_record_and_lock_release_called_false_block_contract(self) -> None:
        missing_record = review_post_execution(self._lock_release(lock_release_record=None))
        not_called = review_post_execution(self._lock_release(lock_release_called=False))

        self.assertEqual("BLOCKED", missing_record["status"])
        self.assertIn("LOCK_RELEASE_RECORD_REQUIRED", missing_record["issues"])
        self.assertEqual("BLOCKED", not_called["status"])
        self.assertIn("LOCK_RELEASE_CALLED_NOT_TRUE", not_called["issues"])
        self.assertFalse(missing_record["runtime_write"])
        self.assertFalse(missing_record["queue_write"])
        self.assertFalse(missing_record["gui_update_called"])

    def test_runtime_queue_gui_lock_update_recorder_and_broker_are_not_recalled(self) -> None:
    ...
    def test_runtime_order_queue_and_rules_hash_unchanged_contract(self) -> None:
