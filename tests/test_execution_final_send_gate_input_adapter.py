...
class ExecutionFinalSendGateInputAdapterTest(unittest.TestCase):
    def _request_preview(self, **overrides: object) -> dict[str, object]:
        preview = {
            "order_id": "ORDER_INPUT_1",
            "source_signal_id": "SIGNAL_INPUT_1",
            "execution_id": "EXEC_INPUT_1",
            "request_hash": "HASH_INPUT_1",
            "lock_id": "LOCK_INPUT_1",
            "account_no": "12345678",
            "side": "BUY",
            "code": "003550",
            "quantity": 10,
    ...
    def test_ready_readiness_builds_ready_input(self) -> None:
        result = self._adapt()
    ...
    def test_blocked_invalid_and_malformed_inputs(self) -> None:
        ...
        self.assertIn("MALFORMED_FINAL_SEND_GATE_READINESS_RESULT", malformed["issues"])

    def test_final_send_gate_input_contains_required_payloads(self) -> None:
        ...
        self.assertEqual(readiness["adapter_preview_result"]["send_order_request_preview"], final_input["send_order_request_preview"])
    ...
    def test_identity_is_preserved(self) -> None:
        ...
        self.assertEqual(self._identity(), result["identity"])
        self.assertEqual(self._identity(), result["final_send_gate_input"]["identity"])
    ...
    def test_missing_payloads_block(self) -> None:
        ...
        for field, issue in (
            ...
            with self.subTest(field=field):
                ...
                broken[field] = None if field != "identity" else {}
                result = self._adapt(readiness=broken)

                self.assertEqual("BLOCKED", result["status"])
                self.assertIn(issue, result["issues"])

    def test_missing_request_preview_blocks(self) -> None:
        readiness = self._readiness()
        broken = deepcopy(readiness)
        broken["adapter_preview_result"]["send_order_request_preview"] = None
    ...
    def test_current_guard_required(self) -> None:
        result = self._adapt(guard={})

        self.assertEqual("BLOCKED", result["status"])
        self.assertIn("CURRENT_GUARD_REQUIRED", result["issues"])
    ...
    def test_deepcopy_independence(self) -> None:
        ...
        self.assertEqual(original_readiness, readiness)
        self.assertEqual("12345678", guard["account_no"])
        self.assertTrue(context["manual_final_send_confirmed"])
