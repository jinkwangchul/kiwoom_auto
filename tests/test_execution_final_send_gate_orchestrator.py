...
class ExecutionFinalSendGateOrchestratorTest(unittest.TestCase):
    def _request_preview(self, **overrides: object) -> dict[str, object]:
        preview = {
            "order_id": "ORDER_ORCH_1",
            "source_signal_id": "SIGNAL_ORCH_1",
            "execution_id": "EXEC_ORCH_1",
            "request_hash": "HASH_ORCH_1",
            "lock_id": "LOCK_ORCH_1",
            "account_no": "12345678",
            "side": "BUY",
            "code": "003550",
    ...
    def test_adapter_ready_yields_orchestrator_ready(self) -> None:
    ...
    def test_adapter_blocked_invalid_and_malformed(self) -> None:
        ...
        self.assertFalse(blocked["final_send_gate_ready"])
    ...
    def test_missing_final_send_gate_input_blocks(self) -> None:
        result = orchestrate_final_send_gate_preview(self._adapter_result(final_send_gate_input=None))

        self.assertEqual("BLOCKED", result["status"])
        self.assertIn("FINAL_SEND_GATE_INPUT_REQUIRED", result["issues"])

    def test_missing_required_payloads_block(self) -> None:
        cases = [
    ...
    def test_missing_nested_adapter_request_preview_blocks(self) -> None:
        final_input = self._final_input(adapter_preview_result=self._adapter_preview(send_order_request_preview=None))

        result = orchestrate_final_send_gate_preview(self._adapter_result(final_send_gate_input=final_input))

        self.assertEqual("BLOCKED", result["status"])
        self.assertIn("ADAPTER_PREVIEW_SEND_ORDER_REQUEST_PREVIEW_REQUIRED", result["issues"])

    def test_ready_input_with_issues_blocks(self) -> None:
    ...
    def test_final_send_gate_input_and_identity_are_preserved(self) -> None:
        adapter_result = self._adapter_result()
        result = orchestrate_final_send_gate_preview(adapter_result)

        self.assertEqual(adapter_result["final_send_gate_input"], result["final_send_gate_input"])
